...
const withAliyunConfigAndroid: ConfigPlugin<AliyunPushConfig> = (
  config,
  {android},
) => {
...
const withAliyunApp: ConfigPlugin<AliyunAndroidConfig | undefined> = (
  config,
  andoridConfig,
) => {
...
const withThirdXiaomi: ConfigPlugin<AndroidThirdConfig['xiaomi']> = (
  ...
  config = withGradleProperties(config, config => {
    config.modResults.push({
      type: 'property',
      key: 'aliyun.push.third.xiaomi.enabled',
...
const withThirdHuawei: ConfigPlugin<AndroidThirdConfig['huawei']> = (
  ...
  config = withGradleProperties(config, config => {
    config.modResults.push({
      type: 'property',
      key: 'aliyun.push.third.huawei.enabled',
      value: String(enabled),
    });
    return config;
  });
...
export default withAliyunConfigAndroid;
